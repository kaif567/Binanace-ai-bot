...
def format_row(window, adx, trades, wr, pf, sq_v2, robustness, drop_top_2):
    run_label = f"ADX >= {adx}" if adx > 0 else "Baseline (ADX 0)"
    return f"| {window:<9} | {run_label:<16} | {trades:<6} | {wr:<5.2f} | {pf:<5.2f} | {sq_v2:<5.2f} | {robustness:<11} | {drop_top_2:<13.2f} |"
...
def run_tests():
    ...
    for w in WINDOWS:
        ...
        for th in THRESHOLDS:
            sys.stderr.write(f"  Running ADX {th} for {w['name']}...\n")
            ...
            if res.get("status") == "failed" or res.get("trades", 0) == 0:
                table_lines.append(format_row(w["name"], th, 0, 0, 0, 0, "FAILED", 0))
            else:
