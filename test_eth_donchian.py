...
WINDOWS = [
    {"name": "Mar 2024 (Bull)", "start": ts(2024, 3, 1), "end": ts(2024, 5, 31)},
    {"name": "Jun 2024 (Chop)", "start": ts(2024, 6, 1), "end": ts(2024, 8, 31)}
]
...
def format_row(window, run_label, trades, wr, pf, robustness, drop_top_2):
    return f"| {window:<15} | {run_label:<10} | {trades:<6} | {wr:<5.2f} | {pf:<5.2f} | {robustness:<11} | {drop_top_2:<13.2f} |"
...
def run_tests():
    print("\n=====================================================================================")
    print("| Window          | Coin       | Trades | WR    | PF    | Robustness  | PF drop top-2 |")
    print("|-----------------|------------|--------|-------|-------|-------------|---------------|")
    
    for w in WINDOWS:
        df = fetch_window("ETHUSDT", w["start"], w["end"])
        history = backtest_donchian(df)
        trades = len(history)
        ...
        print(format_row(w["name"], "ETHUSDT", trades, wr, pf, robustness, ex_pf))
