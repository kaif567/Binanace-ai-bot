...
BATCH2 = {"name": "Mar 2024 (Bull)", "start": ts(2024, 3, 1), "end": ts(2024, 5, 31)}
...
def backtest_donchian(df, trade_amount=100, fee=0.001):
    ...
    # INDEPENDENT SHIFT(1): No lookahead bias, fully contained within this coin's dataframe
...
def run():
    # 1. Recalculate Batch 1 with 95% CI
    b1_n = 228
    b1_wr = 0.3421
    wins_sim = np.random.binomial(b1_n, b1_wr, size=100000)
    sim_gross_profits = wins_sim * 5.0
    sim_gross_losses = (b1_n - wins_sim) * 2.0
    ...
    b1_p025 = np.percentile(sim_pfs, 2.5)
    b1_p975 = np.percentile(sim_pfs, 97.5)
    print("=== Batch 1 (Jun 2024 Chop) Recalculation ===")
    print(f"95% Confidence Interval for PF: [{b1_p025:.2f} to {b1_p975:.2f}]\n")

    # 2. Run Batch 2
    all_trades = []
    print(f"Running Batch 2: {BATCH2['name']} for 4 coins...")
    for coin in COINS:
        df = fetch_window(coin, BATCH2["start"], BATCH2["end"])
    ...
    wins_sim2 = np.random.binomial(n, wr, size=100000)
    sim_gross_profits2 = wins_sim2 * 5.0
    sim_gross_losses2 = (n - wins_sim2) * 2.0
    sim_gross_losses2 = np.where(sim_gross_losses2 == 0, 0.0001, sim_gross_losses2)
    sim_pfs2 = sim_gross_profits2 / sim_gross_losses2
    pf_p025 = np.percentile(sim_pfs2, 2.5)
    pf_p975 = np.percentile(sim_pfs2, 97.5)
    
    print("\n" + "="*80)
    print(f"BATCH 2 RESULTS: {BATCH2['name']} (Portfolio: BTC, ETH, BNB, SOL)")
