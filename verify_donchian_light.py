...
def run_tests():
    ...
    for w in WINDOWS:
        ...
        sys.stderr.write(f"  Running Donchian 20 for {w['name']}...\n")
        ...
        histories[w['name']] = res.get("history", [])

        if res.get("status") == "failed" or res.get("trades", 0) == 0:
            table_lines.append(format_row(w["name"], "Donchian 20", 0, 0, 0, 0, "FAILED", 0))
        else:
    ...
    print("\n[Deep Dive: Root Cause of Fragility]")
    for w_name in ["Aug 2023", "Mar 2024", "Jun 2024"]:
        if w_name in histories:
            history = [t for t in histories[w_name] if "profit" in t]
            if not history:
                continue
